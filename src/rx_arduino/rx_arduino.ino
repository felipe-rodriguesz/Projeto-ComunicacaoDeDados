...
void loop() {
  ...
  if (flag_ler_adc) {
    ...
    uint8_t amostra = ((soma / 3) > limiar_adc) ? 1 : 0;

    // Manchester só é ativado no payload e no CRC (nunca no cabeçalho)
    bool modo_manchester = (codificacao == COD_MANC) &&
                           (estado == LENDO_PAYLOAD || estado == LENDO_CRC);

    if (!modo_manchester) {
      // ══ NRZ-L: 1 tick = 1 bit ══════════════════════════════════════════
      byte_montado = (byte_montado << 1) | amostra;
      if (++bit_count == 8) {
      ...
      if (manc_half == 0) {
        // --- 1ª metade: apenas armazena, ainda não decodifica ---
        manc_first_val = amostra;
        manc_half      = 1;

      } else {
        // --- 2ª metade: decodifica o par (1ª, 2ª) ---
        uint8_t bit_dec;

        if      (manc_first_val == 0 && amostra == 1)  bit_dec = 1; // Baixo→Alto = '1'
        else if (manc_first_val == 1 && amostra == 0)  bit_dec = 0; // Alto→Baixo = '0'
        else {
          // Símbolo Manchester inválido (ruído ou desalinhamento)
          // Usa o valor da 1ª metade como fallback para não travar a FSM.
          // O CRC vai capturar este erro de qualquer forma.
          bit_dec = manc_first_val;
        }
